---
layout: post
title: Fauna y Flora
description: 🐦 Fauna y flora del parque
image: assets/images/fauna_y_flora.png
nav-menu: true
use-initials: true
accent: color-pradera
permalink: /fauna-y-flora
---

<p id="initial-paragraph">Nuestro parque es hogar de muchas especies increíbles:</p><br>
- [Abubilla](/ave/abubilla-comun)
- [Buitre negro](/ave/buitre-negro)
- [Paloma torcaz](/ave/paloma-torcaz)
- [Pico Picapinos](/ave/pico-picapinos)
- [Rabilargo ibérico](/ave/rabilargo-iberico)
- [Urraca común](/ave/urraca-comun)

Cada una juega un papel clave en el ecosistema ¡Trae tus prismáticos y disfruta del espectáculo!